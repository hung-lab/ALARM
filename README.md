...
> RJ. Lung Cancer absolute risk models for mortality in an Asian
> population using the China Kadoorie Biobank. JNCI: Journal of the
> National Cancer Institute. 2022 Dec 1;114(12):1665-73.
...
The `{flexsurv}` models that are used in this package to make
predictions were fitted using a version of the `{flexsurv}` package that
is not compatible with the current release of `{flexsurv}` on CRAN. In
order to use `{ALARM}`, you must install a specific version of
`{flexsurv}` (version 2.2 or 2.2.1).
